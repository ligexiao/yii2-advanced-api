...
class ActiveController extends Controller
{
    ...
    /* Support Get parameters in rest query.
    ...
    public function actionSearch()
    {
        if (!empty($_GET)) {
            ...
            foreach($_GET as $key=>$val){
                ...
                if(isset($fields[$key]) && is_object($fields[$key])){
                    throw new \yii\web\HttpException(404, 'Invalid attribute:' . $key."(key can not be an object!)");
                }
                // covert user_key to db_key
                if(!empty($fields[$key])){
                    $key = $fields[$key];
                }
                $getParams[$key] = $val;
